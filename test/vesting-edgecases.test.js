...
describe('TokenVesting edge cases', function () {
  it('should revert when duration is zero or cliff > duration', async function () {
    ...
    await expect(
      Vesting.deploy(jaja.address, beneficiary.address, now, 10, 0, false)
    ).to.be.revertedWith('Vesting: duration is 0');
    await expect(
      Vesting.deploy(jaja.address, beneficiary.address, now, 100, 10, false)
