...
describe("JajaToken", function () {
  ...
  it("should deploy and set initial supply", async function () {
    ...
    await expect(jaja.connect(addr1).mint(addr1.address, 1)).to.be.revertedWith("JajaToken: must have MINTER_ROLE to mint");
    ...
    await expect(jaja.connect(addr1).pause()).to.be.revertedWith("JajaToken: must have PAUSER_ROLE to pause");
    ...
    await expect(jaja.connect(addr1).transfer(addr1.address, 1)).to.be.revertedWithCustomError(jaja, 'EnforcedPause');
  ...
  it("should respect cap and burn/pause functionality", async function () {
    ...
    await expect(jaja.mint(addr2.address, 1)).to.be.revertedWithCustomError(jaja, 'ERC20ExceededCap');
    ...
    await expect(jaja.connect(addr1).pause()).to.be.revertedWith("Ownable: caller is not the owner");
    await expect(jaja.connect(addr1).unpause()).to.be.revertedWith("Ownable: caller is not the owner");

    // Pause blocks transfers
    await jaja.pause();
    await expect(jaja.connect(addr1).transfer(addr2.address, 1)).to.be.revertedWithCustomError(jaja, 'EnforcedPause');
    await jaja.unpause();
