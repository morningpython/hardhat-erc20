...
describe("Roles & UUPS protection", function () {
  ...
  it("should only allow MINTER_ROLE to mint in upgradeable token", async function () {
    ...
    await expect(jaja.connect(other).mint(other.address, 1)).to.be.revertedWith("JajaToken: must have MINTER_ROLE to mint");
  ...
  it("should only allow owner to authorize upgrades (UUPS) for token and vesting", async function () {
    ...
    const cap = ethers.parseEther("2000");
    const Jaja = await ensureAttach(await ethers.getContractFactory("JajaTokenUpgradeable"));
    const tokenProxy = await upgrades.deployProxy(Jaja, [initialSupply, cap], { initializer: 'initialize' });
    ...
    const proxyAsUUPS = new ethers.Contract(tokenProxy.address, ['function upgradeTo(address) external'], other);
  ...
  it("should enforce PAUSER_ROLE for pause/unpause", async function () {
    ...
    await expect(jaja.connect(other).pause()).to.be.revertedWith("JajaToken: must have PAUSER_ROLE to pause");
    ...
    await expect(jaja.connect(other).transfer(other.address, 1)).to.be.revertedWithCustomError(jaja, 'EnforcedPause');
